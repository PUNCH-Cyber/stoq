...
class TestHelpers(asynctest.TestCase):
    def setUp(self) -> None:
    ...
    async def test_dict_merge(self):
        d1 = defaultdict(list)
        ...
        self.assertEqual(
            await helpers.merge_dicts(d1, d2),
            {
    ...
    async def test_get_md5(self):
        h = await helpers.get_md5(self.generic_content)
        self.assertEqual(h, 'a2004f37730b9445670a738fa0fc9ee5')

    async def test_get_sha1(self):
        h = await helpers.get_sha1(self.generic_content)
        self.assertEqual(h, 'c519c1a06cdbeb2bc499e22137fb48683858b345')

    async def test_get_sha256(self):
        h = await helpers.get_sha256(self.generic_content)
        self.assertEqual(
            h, '5cac4f980fedc3d3f1f99b4be3472c9b30d56523e632d151237ec9309048bda9'
        )

    async def test_get_sha512(self):
        h = await helpers.get_sha512(self.generic_content)
        self.assertEqual(
            h,
